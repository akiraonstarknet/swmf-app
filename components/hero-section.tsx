...
import { EVMSigner } from "./signers/evm-signer"
import { cn, isMainnet, MESSAGE, shortAddress, SUPPORTED_CHAINS } from "@/lib/utils"
...
import { byteArray, Contract, num, RpcProvider, TransactionExecutionStatus } from "starknet"
...
export function HeroSection() {
  ...
  const [snWindowObject, setSnWindowObject] = useState<StarknetWindowObject | null | undefined>(null);
  const [minting, setMinting] = useState(false);
  const [mintCompleted, setMintCompleted] = useState(false);
  const [sourceTransaction, setSourceTransaction] = useState<string | undefined>("abc");
  const [sourceUserAddress, setSourceUserAddress] = useState<string | undefined>("0x1");
  ...
  const provider = new RpcProvider({
    nodeUrl: `https://free-rpc.nethermind.io/${isMainnet() ? 'mainnet' : 'sepolia'}-juno/`
  });
    ...
    while (true) {
      try {
        ...
        if (receipt.isSuccess()) {
          ...
          setMinting(false);
          break;
        } else if(receipt.statusReceipt == 'rejected' || receipt.statusReceipt == 'error' || receipt.statusReceipt == 'reverted') {
          setMinting(false);
          break;
        }
  ...
  return (
    <div className="flex flex-col items-center text-center mt-8 mb-12 max-w-4xl mx-auto">
      {/* Main Heading */}
      <h1 className="text-4xl font-bold mb-4">The First L2 to Settle on Both Bitcoin & Ethereum</h1>
      ...
      <div className="w-full max-w-2xl rounded-lg p-6 border border-gray-800">
        <p className="text-lg mb-6">Send the below message on your preferred chain and mark this historic day.</p>
        ...
        <ol className="flex items-center w-full text-sm font-medium text-center text-gray-500 dark:text-gray-400 sm:text-base mb-[20px]">
            <li className={`flex md:w-full items-center ${activeStep == 1 ? activeClass : ""} sm:after:content-[''] after:w-full after:h-1 after:border-b after:border-gray-200 after:border-1 after:hidden sm:after:inline-block after:mx-6 xl:after:mx-10 dark:after:border-gray-700`}>
            ...
            <li className={`flex items-center min-w-[100px] ${activeStep == 2 ? activeClass : ""}`}>
        ...
        {activeStep == 1 && <div className="mb-6">
          <select
            value={selectedChain}
        ...
        {activeStep == 1 && <div>
          <EVMSigner onSuccess={onSuccessStep1}/>
        </div>}
        {activeStep == 2 && <div className='flex w-full gap-2 items-center justify-center'>
          <button
              className={cn(
          ...
          >
          {snAddress ? `Starknet: ${shortAddress(snAddress)}` : "Connect Starknet Wallet"}
          </button>
          {snAddress && <button 
              className={`flex gap-4 items-center bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-8 rounded-lg transition-colors mb-8 ${minting || mintCompleted ? "disabled:bg-green-700/40 disabled:cursor-not-allowed disabled:text-white/40" : ""}`}
              onClick={mintNFT}
              disabled={minting || mintCompleted}
          >
              Mint NFT
              {minting && <div role="status">
